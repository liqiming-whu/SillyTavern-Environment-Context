...
- 角色卡绑定：设置页顶部支持多选；留空表示全部角色卡均注入。角色列表会在角色数据加载后自动刷新；当前角色从 SillyTavern 实时活动角色状态解析，排除系统占位身份；当前角色与绑定对象不一致时，状态栏会显示双方名称。
...
自动测试共 41 项，覆盖设置归一化、天气自动容错、地址自动容错、缓存隔离、中国/国际日历、非当天纪念日、双对象经期、孕期、动态角色列表、数组下标/avatar/UUID/对象引用角色解析、旧设置迁移、宏模式、三类注入分块和 UI 必需字段。
