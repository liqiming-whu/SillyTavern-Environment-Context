...
- 纪念日：支持 `{{user}}` 生日、`{{char}}` 生日，以及任意名称和日期的自定义纪念日；留空字段不注入。
- 经期：手动设置对象、任意一次已知经期起始日期、周期时间和经期持续时间；插件自动推算最近一次经期开始日和当前阶段。
- 孕期：手动设置对象和怀孕起始日期，自动计算孕周、孕期阶段、状态和预产期；留空不注入。孕期有效时不重复注入经期状态。
...
生日：今天是{{user}}的生日（<年龄>岁）！
纪念日：今天是<自定义纪念日名称>（第<N>年）！
地点：<城市> / <省州地区> / <国家>
天气：<天气状况>
温度：<温度>°C（体感：<体感温度>°C）
湿度：<湿度>%
...
角色生理状态：
- {{user}}：<经期阶段与自然状态描述>
（生理状态应自然地影响精力、情绪和行为，但不必每次都明确提及）
孕期追踪：
- {{char}}：孕<孕周>周（第<孕期阶段>孕期），<孕期状态>，预产期<YYYY-MM-DD>
（孕期状态应自然影响体力、情绪、行动偏好与风险承受，不需要生硬医学播报）
电量：<百分比>%
充电状态：<充电中/未充电>
设备信息：
设备名称：<自定义设备名称或平台通用名称>
...
实际只输出已启用且有有效数据的条目；生日、纪念日只在对应月日注入，经期和孕期字段留空时不注入。
...
自动测试覆盖设置归一化、天气自动容错、地址自动容错、缓存隔离、中国/国际日历、纪念日、经期、孕期、角色卡绑定、宏模式和 UI 必需字段。
