...
4. 禁止重新引入 Termux:API、Android 私有电量属性或服务端子进程。电量每次生成直接从浏览器读取，不缓存；自动定位在 UI 层缓存，默认 10 分钟。反向地址解析支持 auto/Nominatim/BigDataCloud/Photon；auto 顺序固定为 Nominatim → BigDataCloud → Photon，地址缓存键必须包含供应商。
